...
// Audio assets (local files in the same folder)
const bgAudioSrc = 'background sound untill fail.mp3';
const failAudioSrc = 'fail sound.mp3';
const bgAudio = new Audio(bgAudioSrc);
bgAudio.loop = true;
bgAudio.volume = 0.55;
...
function stopBackground() {
  if (!bgAudio) return;
  try { bgAudio.pause(); bgAudio.currentTime = 0; } catch (e) { console.warn(e); }
}

function playFail() {
  if (!failAudio) return;
  try { failAudio.currentTime = 0; failAudio.play().catch(e => console.warn('failAudio play blocked', e)); } catch (e) { console.warn(e); }
}

// Visual assets: background and pillar images
...
function resetGame() {
  ...
  state.pipes = [];
  messageEl.textContent = 'Tap, click, or press Space to fly.';
  updateHud();
  // start background audio when the game starts (user interaction required by some browsers)
  try { playBackground(); } catch (e) { console.warn('playBackground failed', e); }
}
...
function endGame() {
  state.gameOver = true;
  state.running = false;
  messageEl.textContent = 'Game Over — click or tap to restart';
  updateHud();
  // stop background audio and play fail sound
  try { stopBackground(); } catch (e) { console.warn('stopBackground failed', e); }
  try { playFail(); } catch (e) { console.warn('playFail failed', e); }
}
...
function drawBird() {
  ...
  if (birdLoaded && processedBirdImage) {
    ...
    // if processing didn't produce an image but the raw image is available, draw it
...
function render() {
  ...
  if (!state.running && !state.gameOver && state.frame === 0) {
    ...
    ctx.fillText('Click or tap to start', width / 2, height / 2 - 40);
    ctx.font = '16px Inter, system-ui, sans-serif';
    ctx.fillText('Use the attached image as bird.png for the sprite', width / 2, height / 2 + 4);
  }
}
...
messageEl.textContent = 'Click or tap to start';
updateHud();
loop();
